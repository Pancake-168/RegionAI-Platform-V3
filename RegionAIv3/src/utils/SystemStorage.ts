...
class SystemStorage {
    ...
    async setActiveUsername(username: string): Promise<boolean> {
        return await setSetting(this.ksyActiveUsername, this.requireUsername(username))
    }
    ...
    // 设置自动登录完成标记
    async setAutoLoginCompleted(username: string, completed: boolean): Promise<boolean> {
    ...
    // 获取首次分类完成标记
    async getFirstClassificationCompleted(username: string): Promise<boolean | null> {
        const v = await getSetting<boolean>(
            this.buildUsernameScopedKey(username, this.fieldFirstClassificationCompleted),
        )
        return v ?? null
    }
    ...
    // 设置首次分类完成标记
    async setFirstClassificationCompleted(username: string, completed: boolean): Promise<boolean> {
        return await setSetting(
            this.buildUsernameScopedKey(username, this.fieldFirstClassificationCompleted),
            completed,
        )
    ...
    // 设置 Matrix 登录配置字符串
    async setMatrixLoginConfig(username: string, config: string): Promise<boolean> {
        return await setSetting(
            this.buildUsernameScopedKey(username, this.fieldMatrixLoginConfig),
            config,
        )
    ...
    // 设置 Matrix 访问令牌
    async setMatrixAccessToken(username: string, token: string): Promise<boolean> {
        return await setSetting(
            this.buildUsernameScopedKey(username, this.fieldMatrixAccessToken),
            token,
        )
    ...
    // 获取 Matrix 登录配置原始对象
    async getMatrixLoginConfigRaw<T = unknown>(username: string): Promise<T | null> {
    ...
    // 设置 Matrix 登录配置原始对象
    async setMatrixLoginConfigRaw(username: string, config: unknown): Promise<boolean> {
        return await setSetting(
            this.buildUsernameScopedKey(username, this.fieldMatrixLoginConfigRaw),
            config,
        )
    ...
    // 获取当前选中的组织
    async getCurrentOrganization<T = unknown>(username: string): Promise<T | null> {
        const v = await getSetting<T>(
    ...
    // 设置当前选中的组织
    async setCurrentOrganization(username: string, value: unknown): Promise<boolean> {
        return await setSetting(
            this.buildUsernameScopedKey(username, this.fieldCurrentOrganization),
            value,
        )
    ...
    // 获取组织详情列表
    async getOrganizationDetailsList<T = unknown>(username: string): Promise<T | null> {
        const v = await getSetting<T>(
    ...
    // 设置组织详情列表
    async setOrganizationDetailsList(username: string, value: unknown): Promise<boolean> {
        return await setSetting(
            this.buildUsernameScopedKey(username, this.fieldOrganizationDetailsList),
            value,
        )
    ...
    // 获取 bot 思考记录缓存
    async getAgentThinkingCache<T = unknown>(username: string): Promise<T | null> {
        const v = await getSetting<T>(
    ...
    // 设置 bot 思考记录缓存
    async setAgentThinkingCache(username: string, value: unknown): Promise<boolean> {
        return await setSetting(
            this.buildUsernameScopedKey(username, this.fieldAgentThinkingCache),
            value,
        )
    ...
    // 按用户名隔离的通用字段读取
    async getUserField<T = unknown>(username: string, fieldName: string): Promise<T | null> {
        const v = await getSetting<T>(
    ...
    // 按用户名隔离的通用字段写入
    async setUserField(username: string, fieldName: string, value: unknown): Promise<boolean> {
        return await setSetting(
            this.buildUsernameScopedKey(username, fieldName),
            value,
        )
