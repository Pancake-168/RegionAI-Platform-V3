 ...
 *   └─ registerTauriTransport()  → Tauri IPC 写入 regionai.app.log
 *
 * Tauri 集成：
 * - 浏览器开发：仅控制台输出，不写文件
 * - Tauri 桌面端：通过 invoke("write_log") 将日志发送到 Rust 层，
 *   由 Rust 层写入应用自己的 data/logs/ 目录
...
const runtimeFlags = globalThis as {
  __regionaiTauriTransportRegistered?: boolean
  __regionaiGlobalErrorHandlersRegistered?: boolean
}

// ---- 格式化函数 ----

/** 格式化日志前缀：[RegionAI:fileName:functionName]message */
const formatLogPrefix = (
  fileName: string,
  functionName: string,
  message: string,
) => `[RegionAI:${fileName}:${functionName}]${message}`
 ...
 * Tauri 环境将日志通过 IPC 发送到 Rust 层，写入应用自己的 data/logs/ 目录。
 */
export const registerTauriTransport = () => {
  if (runtimeFlags.__regionaiTauriTransportRegistered) {
    return
  }
...
export const registerGlobalErrorHandlers = () => {
  if (runtimeFlags.__regionaiGlobalErrorHandlersRegistered) {
    return
  }
