...
export const useLanguageStore = defineStore("language", () => {
  ...
  async function setLanguage(lang: string): Promise<boolean> {
    ...
    logInfo(
      "language",
      "",
      "[System:Language:setLanguage] 可以使用的语言列表:",
      list,
    );
    if (!list.includes(lang)) {
      logWarn(
        "language",
        "",
        `[System:Language:setLanguage] "${lang}" 没有注册。可用的语言有: ${list.join(", ")}`,
      );
      return false;
    }

    const globalInstance = i18n.global as unknown as {
      locale: { value: string } | string;
    };
    if (
      typeof globalInstance.locale === "object" &&
      "value" in globalInstance.locale
    ) {
    ...
    try {
      ...
      if (!username) {
        logWarn(
          "language",
          "",
          "[System:Language:setLanguage] 无法获取当前用户名，无法保存语言设置到服务器",
        );
        return true;
      }
      const localConfig =
        await SystemStorageManager.getUserConfig<UserConfig>(username);
      const nextConfig: UserConfig = { ...(localConfig ?? {}), language: lang };
      await SystemStorageManager.setUserConfig(username, nextConfig);
  ...
  async function initLanguage() {
    try {
      ...
      if (saved && getLanguageList().includes(saved)) {
        ...
        const detected =
          (typeof globalInstance.locale === "object"
            ? globalInstance.locale.value
            : globalInstance.locale) || defaultLanguage;
